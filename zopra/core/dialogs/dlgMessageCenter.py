...
SORT_FIELDS    = [TCN_READ, TCN_SUBJECT, 'entrydate', TCN_SSENDER]
...
class dlgMessageCenter( hgDialog, guiHandler ):
    """\class dlgMessageCenter

    \brief Folder management for Messaging Manager.
    """
    _className = 'dlgMessageCenter'
    ...
    def __init__( self, manager, param_dict ):
        ...
        # dlg private data
        self.data = dlgMessageCenterPrivate()

        # init values
        self.resetData()
        
        # select the current folder
        folder = param_dict.get('folder', manager.FLD_INBOX)

        if folder in [manager.FLD_INBOX, manager.FLD_TRASH]:
            self.folder = folder
        else:
            try:
                folder = int( folder )
            ...
            self.folder = manager.tableHandler[TN_FOLDER].getEntry(folder)
        ...
        # init buttons
        self.data.delFldButton      = hgPushButton( parent = self, name = 'delete',      icon = del_icon         )
        ...
        self.data.delFldButton.setText      ( '&Delete Folder'  )
        ...
        self.data.delFldButton.setToolTip      ( 'Delete Folder'   )
        ...
        self.connect( self.data.delFldButton.clicked,            self.deleteFld )
    ...
    def getMessages(self, manager):
        ...
        # get msg count
        if self.folder == manager.FLD_INBOX:
            ...
            constraints = { TCN_FOLDER: self.folder[TCN_AUTOID], 
                            TCN_TRASH: 0 }

        if self.threadview:
            pass
            
        self.msgcount = manager.tableHandler[TN_LOCAL].getRowCount( constraints = constraints )
            
        # get messages
        self.messages = manager.tableHandler[TN_LOCAL].getEntryList( show_number  = self.entriespp,
...
#        self.msgCountNew   = -1
#        self.msgCountTotal = -1
        
        # TODO: nice layout
        for folder in manager.SYSTEM_FOLDER:
            if folder == self.folder:
                self.msgCountNew   = system_count[folder][0]
                self.msgCountTotal = system_count[folder][1]
                start = '<b>'
                end   = '</b>'
            else:
                start = ''
        ...
        for folder in folders:
            # NOTE: due to the ZMOM internal dlg class renaming, 
            #       which is just string replacement in the dlg class file
            #       we need to prevent unwanted replacment of class name usage 
            #       therefore the split up of the original string into 'dlg' + 'MessageCenter'
            uri = '%s/dlgHandler/dlg' % url + 'MessageCenter/show?folder=%s' % str(folder[TCN_AUTOID])
            ...
            if not self.folder in [manager.FLD_INBOX, manager.FLD_TRASH] and \
               folder[TCN_AUTOID] == self.folder[TCN_AUTOID]:
                self.msgCountNew   = new
                self.msgCountTotal = total
                start = '<b>'
                end   = '</b>'
            else:
                start = ''
            ...
            label = start + folder[TCN_FOLDERNAME] + ' (' + str(new) + '/' + str(total) + ')' + end
    ...
    def layoutFolder(self, manager, widget = None, parent = None):
        ...
        # print foldername
        if self.folder in [ manager.FLD_INBOX, manager.FLD_TRASH ]:
            foldlab = '<b>' + self.folder + '</b>'
        else:
            foldlab = '<b>' + self.folder[TCN_FOLDERNAME] + '</b>'
        
        if self.threadview:
            foldlab += ' (Thread View)'

        wlay.addWidget( hgLabel(foldlab, parent = widget), 0, c )
        c += 1
        ...
        # unread msg count 
        if self.msgCountTotal == 1:
            countlab = str(self.msgCountTotal) + ' Message (' + str(self.msgCountNew) + ' unread)'
        else:
            countlab = str(self.msgCountTotal) + ' Messages (' + str(self.msgCountNew) + ' unread)'
        wlay.addWidget( hgLabel(countlab, parent = widget), 0, c )
        c += 1

        
        # delete button
        if not self.folder in manager.SYSTEM_FOLDER and \
           self.msgcount == 0:
            wlay.addWidget( hgLabel('&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;', parent = widget), 0, c )
            c += 1
            self.data.delFldButton.reparent(widget)
            wlay.addWidget(self.data.delFldButton, 0, c, hg.AlignRight )
            c += 1
    ...
    def layoutFunctions(self, manager, widget = None, parent = None):
        ...
        if self.folder == manager.FLD_TRASH:    
            self.data.actionCB.insertItem('------------', '')
            self.data.actionCB.insertItem(DLG_ACTLABEL[RESTORE_TRASH], RESTORE_TRASH   )
    ...
    def displayMessages(self, manager):
        ...
        for i, msg in enumerate(self.messages):
            ...
            msgin  = manager.tableHandler[TN_LOCAL].getRowCount( constraints )
            constraints = { TCN_THREAD:  msg[TCN_THREAD],
                            TCN_SSENDER: self.muser
                          }
            ...
            # subject
            link = manager.getLink(TN_LOCAL, None, msg, parent = tab)
    ...
    def markAsRead(self, manager, id, read = True):
        ...
        assert ( self.aid2msg.has_key(id) )
        if self.aid2msg[id].get(TCN_READ, 0) != int(read):
    ...
    def execDlg(self, manager, REQUEST):
        ...
        if self.result() == self.New:
            ...
            self.setResult(self.Running)
        elif self.result() == self.DelFolder:
            self.performDelFolder(manager)
    ...
    def setDefaultTarget(self, manager):
        """"""

        # set dlg target
        url    = manager.absolute_url()
        # NOTE: prevent class name in target string from being replaced by ZMOM
        target = '%s/dlgHandler/dlg'  % (url) + 'MessageCenter/show'
    ...
    def performDelFolder(self, manager):
        """\reimp"""
        
        if self.folder in [manager.FLD_TRASH, manager.FLD_INBOX]:
            self.setResult(self.Running)
            return
            
        manager.deleteEntries( TN_FOLDER, [ self.folder ] )
        
        self.setDefaultTarget(manager)
    ...
    def performDelete(self, manager):
        ...
        if msg_id:
            ...
            if self.folder == manager.FLD_TRASH:
                manager.deleteEntries( TN_LOCAL, [msg_id] )
            else:
                msg = self.aid2msg[msg_id]
                msg[TCN_TRASH] = 1
                
                if not manager.tableHandler[TN_LOCAL].updateEntry(msg, msg[TCN_AUTOID]):
    ...
    def performGo(self, manager):
        ...
        if self.action in [ MARK_READ, MARK_UNREAD ]:
            ...
            for aid in msg_ids:
                ...
                self.aid2msg[aid][TCN_READ] = read
        ...
        # empty trash
            ...
            if self.folder == manager.FLD_TRASH:
                self.start = 0
        # restore from trash
        elif self.action == RESTORE_TRASH:
            if self.folder == manager.FLD_TRASH:
                for aid in msg_ids:
                    msg = self.aid2msg[aid]
                    msg[TCN_TRASH] = 0
                    if not manager.tableHandler[TN_LOCAL].updateEntry(msg, msg[TCN_AUTOID]):
                        manager.displayError('An error occured while restoring message from trash.', 
                                             'Database error')                        
        # delete messages
        elif self.action == DELETE_MSG:
            manager.deleteEntries( TN_LOCAL, msg_ids )
