...
if not entry.get("iscopyof"):
    message = _(
        "zopra_editconfirm_abort_no_copy",
        default=u"Only working copies can be published. This entry is up-to-date. Publishing was cancelled.",
    )
...
if not origentry:
    message = _(
        "zopra_editconfirm_abort_not_found",
        default=u"Original entry could not be found. Publishing was cancelled.",
    )
    context.plone_utils.addPortalMessage(message, "info")
    return state.set(status="failure", context=context)
...
if (
    ...
    or origentry.get("language") == context.lang_default
):
...
if done == True:
    ...
    if done == True:
        ...
        message = _(
            "zopra_editconfirm_almost_success",
