    ...
    def getSQL(
        ...
        # FIXME: joined multilists spoil count functions, should use grouping here too
            ...
            if function.find("%s") != -1:
                function = function % ("%s%s." % (self.mgr.id, self.data.tablename))
        ...
        # col_list is given, use it for selection
            ...
            for column in col_list:
                ...
                if column == ZC.TCN_AUTOID:
                    ...
                    attr = "%s%s.%s" % (self.mgr.id, self.data.tablename, column)
                    sellist.append(attr)
                elif do_group:
                    ...
                    attr = "min(%s%s.%s)" % (self.mgr.id, self.data.tablename, column)
                    sellist.append(attr)
                else:
                    # normal generation
                    attr = "%s%s.%s" % (self.mgr.id, self.data.tablename, column)
            ...
            if do_group or (order and dist):
                ...
                if ZC.TCN_AUTOID not in col_list:
                    attr = "%s%s.%s" % (self.mgr.id, self.data.tablename, ZC.TCN_AUTOID)
                    sellist.append(attr)
        ...
        if do_group:
            grpstr = "%s%s.%s" % (self.mgr.id, self.data.tablename, ZC.TCN_AUTOID)
            finalsql = "%s%s GROUP BY %s"
            finalsql = finalsql % (select, self.sql, grpstr)
        else:
            finalsql = "%s%s" % (select, self.sql)

        if not function:
            # generate the string part for order limit and offset,
            olo = self.generateOrderLimitOffset(order, limit, offset)
