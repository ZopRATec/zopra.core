(function($){
    ...
    {
        var id = undefined
        while (id == undefined || $("#ms-"+id).length != 0)
  ...
  var msMethods = {
    ...
    'init' : function(options){
      ...
      multiSelects.each(function(){
        ...
        //ms.attr('id', ms.attr('id') ? ms.attr('id') : getUniqueId());
    ...
    'deselect' : function(value){
      ...
      if(present){
        ...
        if (newValues.length == 0){
        	// selecting the empty value when last item was deselected lead to strangest behaviours in the templates / controller scripts
            // we really select nothing when nothing is selected
          newValues = [];//[''];
        }
        ms.val(newValues);
        selectableLi.show();
