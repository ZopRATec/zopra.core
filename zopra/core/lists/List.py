...
class List(GenericList):
    ...
    def addValue(self, value, notes="", rank=None, show="yes", **kwargs):
        ...
        if result is None:
            ...
            message = "add entry %s to %s" % (entry_id, my_id + self.listname)
    ...
    def delValue(self, autoid):
        ...
        # now delete
        mgr.getManager(ZC.ZM_PM).simpleDelete(mgr.id + self.listname, autoid)
        self.clearCache()
    ...
    def getAutoidByValue(self, value, rank=None, ignore_cache=False):
        ...
        assert (
            ...
            or value is None
        ), "Wrong type for parameter value: %s [%s]" % (value, value.__class__)
