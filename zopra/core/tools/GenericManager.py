...
class GenericManager(ManagerPart):
    ...
    """ usage: _generic_config = {table: {key:value}}
        keys: basket_to     (True / False) - for showForm/showList-BasketButton
              ...
              specials      ([{'label': '<label>', fieldname: function}] - call the module function on every displayed entry in search result listing (editorial) to calculate the html content for a special column, label is used for table head, function parameters are: mgr, table, entry, lang
