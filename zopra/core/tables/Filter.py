...
def checkTypeDeprecated(value, column_type, operator = False, label = None, do_replace = True):
    ...
    # lists for freetext-search-results
        ...
        if '_not_ NULL' in value or '_not_NULL' in value:
            ...
            entry_list = map( lambda onevalue: checkTypeDeprecated( onevalue,
                                                       column_type,
                                                       False,
                                                       label ),
                              value
    ...
    # we allow range searches with keyword _to_
        ...
        value = "%s and %s" % ( checkTypeDeprecated( value[:pos_to].rstrip(),
                                           column_type,
                                           False,
                                           label ),
                                checkTypeDeprecated( value[pos_to + 4:].lstrip(),
                                           column_type,
                                           False,
                                           label )
                                )
        ...
        oper   = 'IN'
        entry_list = map( lambda onevalue: checkTypeDeprecated( onevalue.strip(),
                                                       column_type,
                                                       False,
                                                       label ),
                          valuelist
        ...
        oper  = '<'
        value = checkTypeDeprecated( value[pos_lt + 3:].strip(),
                           column_type,
                           False,
                           label )
    elif pos_lte != -1:
        oper  = '<='
        value = checkTypeDeprecated( value[pos_lte + 4:].strip(),
                           column_type,
                           False,
                           label )
    elif pos_gt != -1:
        oper  = '>'
        value = checkTypeDeprecated( value[pos_gt + 3:].strip(),
                           column_type,
                           False,
                           label )
    elif pos_gte != -1:
        oper  = '>='
        value = checkTypeDeprecated( value[pos_gte + 4:].strip(),
                           column_type,
                           False,
                           label )
    else:
...
class Filter:
    ...
    def setConstraints(self, consdict, prefix = None, tableData = None):
        ...
        for entry in consdict:
            ...
            if len(entry) - 7 == entry.find('_NOT_IN') and entry[:len(entry) - 7] in consdict.keys() and isinstance(consdict[entry[:len(entry) - 7]], ListType):
                ...
                if field:
                    ...
                    if field.get(COL_TYPE) in ['string', 'memo'] and isinstance(consdict[pre + entry], ListType):
                        ...
                        for listentry in consdict[pre + entry]:
                            child = Filter('AND')
                            child.setConstraints( { entry[plen:]: listentry },
                                                  tableData = tableData
                                                  )
                            child.setFinal()
    ...
    def getSQL(self, tablename, mgrId, tableData = None, checker = None):
        ...
        for cons in self.constraints:
            ...
            if ctype not in MULLISTS:
                ...
                value, operator = checkType( self.constraints[cons],
                                             ctype,
                                             True,
                                             cons )
