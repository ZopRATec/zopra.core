...
for col in cols:
    ...
    if header[1].lower() in (col.lower(), coltypes[col]['LABEL'].lower()):
        ...
        break
if attribute == None:
    error = _('zopra_twocol_import_attribute_error',
...
message = _('zopra_twocol_import_count',
...
for index, c in enumerate(content):
    ...
    if not entry:
        message = _('zopra_twocol_import_entry_missing', u'Entry not found for autoid ${autoid} on line ${line}', mapping = {'autoid': autoid, 'line': index + 1})
        context.plone_utils.addPortalMessage(context.translate(message), 'info')
        continue
    ...
    if manager.doesWorkingCopies(table) and manager.updateWorkingCopy(table, entry_diff):
        wc = context.translate(_('zopra_twocol_import_workingcopy', default = u' + working copy'))
    if manager.doesTranslations(table) and manager.updateTranslation(table, entry_diff):
        tr = context.translate(_('zopra_twocol_import_translation', default = u' + translation copy'))
    message = _('zopra_twocol_import_entry_saved', u'- original entry (autoid: ${autoid})${wc}${tr}', mapping = {'autoid': autoid, 'wc': wc, 'tr': tr})
    context.plone_utils.addPortalMessage(context.translate(message), 'info')
message = _('zopra_twocol_import_success', default = u'${count} entries have been updated.', mapping = {'count': done})
context.plone_utils.addPortalMessage(context.translate(message), 'info')
