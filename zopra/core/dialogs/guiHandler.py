...
class guiHandler:
    ...
    \brief Mix-in class for the zopra part of the gui handling.
    """
    ...
    def _foundWidgetFunction( self, widget, value ):
        """\brief This function is called if a \a widget was found that gets
                  a new \a value.
        ...
        elif isinstance(widget, hgLineEdit) or \
             isinstance(widget, hgTextEdit):
    ...
    #_foundWidgetFunction = staticmethod( _foundWidgetFunction )
    ...
    def _processEvents( self, widget, form ):
        ...
        if isinstance(widget, hgRadioButton):
            widget.setChecked(False)
        if isinstance(widget, hgMultiList):
        ...
        if form.has_key(widget.name):
            ...
            # radiobuttons do all have the same name (same can happen for checkboxes)
    ...
    #_processEvents = staticmethod ( _processEvents )


    def getBackButtonStr(REQUEST = None, prop = True):
        """\brief Returns a back button string.

        The function tries to keep track of how often a user should go back to
        a specific site. But it only works if the function gets a REQUEST
        object.
    ...
    def __init__(self, flags = Standalone):
        """\brief Initialise the guiHandler."""
        ...
        # this is highly deprecated. The Property for dialog recognition is set in ZMOMDialogHandler.show(...)
        # still left it in here for backwards compatibility. Have to check where this is used
