...
# basketbuttons
mpfBasketActivateButton   = hgPushButton( BTN_BASKET_ACTIVATE,
                                          DLG_FUNCTION + BTN_BASKET_ACTIVATE )

mpfBasketDeactivateButton = hgPushButton( BTN_BASKET_DEACTIVATE,
                                          DLG_FUNCTION + BTN_BASKET_DEACTIVATE )

mpfBasketRemoveButton     = hgPushButton( BTN_BASKET_REMOVE,
                                          DLG_FUNCTION + BTN_BASKET_REMOVE )

mpfBasketRemoveAllButton  = hgPushButton( BTN_BASKET_REMOVEALL,
                                          DLG_FUNCTION + BTN_BASKET_REMOVEALL )

mpfBasketAddButton        = hgPushButton( BTN_BASKET_ADD,
                                          DLG_FUNCTION + BTN_BASKET_ADD )

mpfBasketPopButton        = hgPushButton( BTN_BASKET_POP,
                                          DLG_FUNCTION + BTN_BASKET_POP )

mpfBasketGetButton        = hgPushButton( BTN_BASKET_GET,
                                          DLG_FUNCTION + BTN_BASKET_GET )
...
def getPressedButton( REQUEST = None, buttonType = DLG_FUNCTION):
    ...
    if REQUEST:
        ...
        for key in REQUEST.form.keys():
            if key[0:length] == buttonType and \
               key[length:] not in button_dict:
...
def getSpecialField(REQUEST = None, fieldType = DLG_CUSTOM):
    ...
    if REQUEST:
        ...
        for key in REQUEST.keys():
            if key[0:length] == fieldType and \
               (key[length:] not in field_dict):
